...
protectedApiRouter.use("/movie", _movie__WEBPACK_IMPORTED_MODULE_6__["router"].routes());
...
router.get("/", async ctx => {
  ...
  response.body = await _store__WEBPACK_IMPORTED_MODULE_1__["movieStore"].find({
    userId
  });
...
router.get("/:id", async ctx => {
  const userId = ctx.state.user._id;
  const movie = await _store__WEBPACK_IMPORTED_MODULE_1__["movieStore"].findOne({
    _id: ctx.params.id
  });
  const response = ctx.response;
  if (movie) {
...
const createMovie = async (ctx, movie, response) => {
  try {
    ...
    movie.userId = userId;
    movie._id = undefined;
    const insertedMovie = await _store__WEBPACK_IMPORTED_MODULE_1__["movieStore"].insert(movie);
    response.body = _objectSpread(_objectSpread({}, insertedMovie), {}, {
      _id: insertedMovie._id
    });
    console.log('response.body', response.body);
    response.status = 201; // CREATED
    Object(_utils__WEBPACK_IMPORTED_MODULE_2__["broadcast"])(userId, {
      type: 'created',
      payload: insertedMovie
    });
  } catch (err) {
    response.body = {
      message: err.message
...
router.put('/sync', async ctx => {
  try {
    ...
    const response = ctx.response;
    const currentMovies = await _store__WEBPACK_IMPORTED_MODULE_1__["movieStore"].find({
      userId
    });
    for (var movie of movies) {
      ...
      if (!movie._id) {
        ...
        if (existingMovie) {
          await _store__WEBPACK_IMPORTED_MODULE_1__["movieStore"].update({
            _id: movie._id
          }, movie);
        }
      }
...
router.put("/:id", async ctx => {
  ...
  if (!movieItemId) {
    ...
    const updatedCount = await _store__WEBPACK_IMPORTED_MODULE_1__["movieStore"].update({
      _id: id
    }, movieItem);
    if (updatedCount === 1) {
      response.body = movieItem;
...
router.del("/:id", async ctx => {
  const userId = ctx.state.user._id;
  const movieItem = await _store__WEBPACK_IMPORTED_MODULE_1__["movieStore"].findOne({
    _id: ctx.params.id
  });
  if (movieItem && userId !== movieItem.userId) {
    ctx.response.status = 403; // forbidden
  } else {
    await _store__WEBPACK_IMPORTED_MODULE_1__["movieStore"].remove({
      _id: ctx.params.id
    });
...
class movieItemStore {
  ...
  async insert(movie) {
    if (movie.name.length < 1) throw new Error('Name must be at least 1 character');
...
const movieStore = new movieItemStore({
...
module.exports = __webpack_require__(/*! C:\Users\daria.marian\Downloads\AndroidServer\AndroidServer\src/index.js */"./src/index.js");
