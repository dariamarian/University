...
namespace proiectSCcsharp.repository
{
    public class RepoBilet : IRepoBilet
    {
        ...
        public Bilet add(Bilet entity)
        {
            ...
            {
                command.CommandText = "INSERT INTO Bilete(cumparator_name, id_spectacol, nr_seats) VALUES (@c, @ids, @n)";
                ...
                idsParameter.Value = entity.id_spectacol;
            ...
            using (var command = connection.CreateCommand())
            {
                ...
                using (var dataReader = command.ExecuteReader())
                {
                    while (dataReader.Read())
                    {
                        ...
                        Bilet bilet = new Bilet(name, ids, seats);
                        bilet.id = id;
                        bilete.Add(bilet);
                    }
                }
            ...
            using (var command = connection.CreateCommand())
            {
                ...
                using (var dataReader = command.ExecuteReader())
                {
                    while (dataReader.Read())
                    {
                        ...
                        long ids = dataReader.GetInt64(2);
                        int seats = dataReader.GetInt32(3);
                        bilet = new Bilet(name, ids, seats);
                        bilet.id = id;
