...
import proiect.persistence.repository.IRepoSpectacol;
...
public class RepoSpectacol implements IRepoSpectacol {
    ...
    public Spectacol add(Spectacol entity) {
        ...
        String query = "INSERT INTO Spectacole(artist_name,date,place,available_seats,sold_seats,time) VALUES(?,?,?,?,?,?)";
    ...
    public void update(Spectacol spectacol, int nrSeats) {
        ...
        try (Connection connection = jdbcUtils.getConnection();
            ...
            statement.setInt(3,spectacol.getId());
            int result=statement.executeUpdate();
    ...
    @Override
    public Spectacol remove(Integer aLong) {
    ...
    @Override
    public Spectacol findElement(Integer aLong) {
        logger.traceEntry();
        ...
        try (Connection connection = jdbcUtils.getConnection();
            ...
            while (resultSet.next()) {
                ...
                spectacol=new Spectacol(idSpectacol, artist_name,date,time,place,available,sold);
            }
        } catch (SQLException e) {
            logger.error(e);
            e.printStackTrace();
    ...
    public List<Spectacol> getAll() {
        ...
        try (Connection connection = jdbcUtils.getConnection();
            ...
            while (resultSet.next()) {
                ...
                int sold = resultSet.getInt("sold_seats");
                String time = resultSet.getString("time");
                Spectacol spectacol=new Spectacol(idSpectacol,artist_name,date,time,place,available,sold);
