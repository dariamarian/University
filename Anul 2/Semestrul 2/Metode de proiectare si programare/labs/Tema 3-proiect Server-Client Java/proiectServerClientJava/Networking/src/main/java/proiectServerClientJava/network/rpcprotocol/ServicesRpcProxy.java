...
import static proiectServerClientJava.domain.utils.Constants.FORMATTER_MESSAGE;
...
public class ServicesRpcProxy implements IService {
    ...
    @Override
    public void addBilet(String cumparatorName, long idSpectacol, int nrSeats) throws MyException {
    ...
    @Override
    public Angajat getAngajatByUsername(String username) throws MyException {
        initializeConnection();
        Request req = new Request.Builder().type(RequestType.GET_ANGAJAT).data(username).build();
        sendRequest(req);
        Response response = readResponse();
        if (response.type() == ResponseType.ERROR) {
    ...
    @Override
    public Spectacol getSpectacolByAll(String artistName, LocalTime time, String place) throws MyException {
        Spectacol spectacol=new Spectacol(artistName,place,0,0);
        LocalDate today = LocalDate.now();
        LocalDateTime dateTime=time.atDate(today);
        spectacol.setDate(dateTime.format(FORMATTER_MESSAGE));
        Request req = new Request.Builder().type(RequestType.GET_SPECTACOL_BY_ALL).data(spectacol).build();
        sendRequest(req);
        Response response = readResponse();
        if (response.type() == ResponseType.ERROR) {
            logger.error("Error finding spectacol" + response.data().toString());
    ...
    @Override
    public Iterable<Angajat> getAllAngajati() throws MyException {
        Request req = new Request.Builder().type(RequestType.GET_ALL_ANGAJATI).data(null).build();
        sendRequest(req);
        Response response = readResponse();
        if (response.type() == ResponseType.ERROR) {
