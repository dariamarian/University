...
public class ClientRpcReflectionWorker implements Runnable, IObserver {
    ...
    private Response handleADD_BILET(Request request){
        ...
        try {
            service.addBilet(bilet.getCumparatorName(),bilet.getIdSpectacol(),bilet.getNrSeats());
    ...
    private Response handleGET_ANGAJAT(Request request){
        logger.traceEntry("method entered: handleGET_ANGAJAT with parameters "+request);
        String username=(String) request.data();
        try {
            Angajat angajat=service.getAngajatByUsername(username);
            logger.info("Angajat found "+angajat);
            return new Response.Builder().type(ResponseType.GET_ANGAJAT).data(angajat).build();
        } catch (MyException e) {
            logger.error("Error in worker (solving method handleGET_ANGAJAT): "+e);
            return new Response.Builder().type(ResponseType.ERROR).data(e.getMessage()).build();
        }
    }
    ...
    private Response handleGET_SPECTACOL_BY_ALL(Request request){
        logger.traceEntry("method entered: handleGET_SPECTACOL_BY_ALL with parameters "+request);
        Spectacol spectacol=(Spectacol) request.data();
        logger.info("Spectacol found "+spectacol);
        try{
            Spectacol spectacol1=service.getSpectacolByAll(spectacol.getArtistName(), spectacol.getDate().toLocalTime(), spectacol.getPlace());
    ...
    private Response handleGET_ALL_ANGAJATI(Request request){
        logger.traceEntry("method entered: handleGET_ALL_ANGAJATI with parameters "+request);
        try {
            List<Angajat> angajati= (List<Angajat>) service.getAllAngajati();
            logger.info("Angajati found "+angajati);
            return new Response.Builder().type(ResponseType.GET_ALL_ANGAJATI).data(angajati).build();
        } catch (MyException e) {
            logger.error("Error in worker (solving method handleGET_ALL_ANGAJATI): "+e);
            return new Response.Builder().type(ResponseType.ERROR).data(e.getMessage()).build();
        }
    }
